class SimulationDataController < ApplicationController
  ...
  def get_this_date_slots
    ...
    # return
    @this_date_slots = @simulation.simulation_datums.where(:performance_date=>@this_date).map{|tds| [tds.id, tds.programme_name, tds.start_time.strftime("%T"), (tds.end_time.strftime("%T") rescue ""),tds.duration]}
